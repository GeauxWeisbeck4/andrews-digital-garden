...
modified: 2024-10-29T11:46:58-04:00
...
![Image](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780134494272/files/graphics/CH-UN05.jpg)
...
Some folks fall back on three magic words to explain the nature of OO: _encapsulation_, _inheritance_, and _polymorphism_. The implication is that OO is the proper admixture of these three things, or at least that an OO language must support these three things.
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex01a)
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex02a)
...
The C++ compiler, for technical reasons,[1](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn1) needed the member variables of a class to be declared in the header file of that class. So our `Point` program changed to look like this:

[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex03a)
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex04a)
...
For these reasons, it is difficult to accept that OO depends on strong encapsulation. Indeed, many OO languages[2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn2) have little or no enforced encapsulation.
...
Well—sort of. Inheritance is simply the redeclaration of a group of variables and functions within an enclosing scope. This is something C programmers[3](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn3) were able to do manually long before there was an OO language.

Consider this addition to our original `point.h` `C` program:

[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex05a)
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex06a)
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex07a)
...
This kind of trickery was a common practice[4](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn4) of programmers prior to the advent of OO. In fact, such trickery is how C++ implements single inheritance.
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex08a)
...
The function `getchar()` reads from `STDIN`. But which device is `STDIN`? The `putchar()` function writes to `STDOUT`. But which device is that? These functions are _polymorphic_—their behavior depends on the type of `STDIN` and `STDOUT`.
...
The answer to that question is pretty straightforward. The UNIX operating system requires that every IO device driver provide five standard functions:[5](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn5) `open`, `close`, `read`, `write`, and `seek`. The signatures of those functions must be identical for every IO driver.

The `FILE` data structure contains five pointers to functions. In our example, it might look like this:

[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex09a)
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex010a)
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5_images.xhtml#pch5ex011a)
...
The problem with explicitly using pointers to functions to create polymorphic behavior is that pointers to functions are _dangerous_. Such use is driven by a set of manual conventions. You have to remember to follow the convention to initialize those pointers. You have to remember to follow the convention to call all your functions through those pointers. If any programmer fails to remember these conventions, the resulting bug can be devilishly hard to track down and eliminate.
...
Why did the UNIX operating system make IO devices plugins? Because we learned, in the late 1950s, that our programs should be _device independent_. Why? Because we wrote lots of programs that were device _dependent_, only to discover that we really wanted those programs to do the same job but use a different device.

For example, we often wrote programs that read input data from decks of cards,[6](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn6) and then punched new decks of cards as output. Later, our customers stopped giving us decks of cards and started giving us reels of magnetic tape. This was very inconvenient, because it meant rewriting large portions of the original program. It would be very convenient if the same program worked interchangeably with cards or tape.
...
When polymorphism is brought into play, however, something very different can happen ([Figure 5.2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fig2)).

![Image](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780134494272/files/graphics/05fig02.jpg)

**Figure 5.2** Dependency inversion

In [Figure 5.2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fig2), module `HL1` calls the `F()` function in module `ML1`. The fact that it calls this function through an interface is a source code contrivance. At runtime, the interface doesn’t exist. `HL1` simply calls `F()` within `ML1`.[7](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn7)

Note, however, that the source code dependency (the inheritance relationship) between `ML1` and the interface `I` points in the opposite direction compared to the flow of control. This is called _dependency inversion_, and its implications for the software architect are profound.

The fact that OO languages provide safe and convenient polymorphism means that _any source code dependency, no matter where it is, can be inverted_.

Now look back at that calling tree in [Figure 5.1](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fig1), and its many source code dependencies. Any of those source code dependencies can be turned around by inserting an interface between them.

With this approach, software architects working in systems written in OO languages have _absolute control_ over the direction of all source code dependencies in the system. They are not constrained to align those dependencies with the flow of control. No matter which module does the calling and which module is called, the software architect can point the source code dependency in either direction.
...
What can you do with that power? As an example, you can rearrange the source code dependencies of your system so that the database and the user interface (UI) depend on the business rules ([Figure 5.3](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fig3)), rather than the other way around.

![Image](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780134494272/files/graphics/05fig03.jpg)
...
In turn, the business rules can be _deployed independently_ of the UI and the database. Changes to the UI or the database need not have any effect on the business rules. Those components can be deployed separately and independently.

In short, when the source code in a component changes, only that component needs to be redeployed. This is _independent deployability_.

If the modules in your system can be deployed independently, then they can be developed independently by different teams. That’s _independent developability_.
...
[1](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn-1). The C++ compiler needs to know the size of the instances of each class.

[2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn-2). For example, Smalltalk, Python, JavaScript, Lua, and Ruby.

[3](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn-3). Not just C programmers: Most languages of that era had the capability to masquerade one data structure as another.

[4](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn-4). Indeed it still is.

[5](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn-5). UNIX systems vary; this is just an example.

[6](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn-6). Punched cards—IBM Hollerith cards, 80 columns wide. I’m sure many of you have never even seen one of these, but they were commonplace in the 1950s, 1960s, and even 1970s.

[7](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch5.xhtml#ch5fn-7). Albeit indirectly.