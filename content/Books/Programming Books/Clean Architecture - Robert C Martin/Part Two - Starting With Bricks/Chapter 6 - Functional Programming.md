...
modified: 2024-10-29T11:50:52-04:00
...
![Image](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780134494272/files/graphics/CH-UN06.jpg)
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch6_images.xhtml#pch6ex01a)
...
[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch6_images.xhtml#pch6ex02a)

(println (take 25 (map (fn [x] (* x x)) (range))))

If you don’t know Lisp, then this might look a little strange. So let me reformat it a bit and add some comments.

[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch6_images.xhtml#pch6ex03a)
...
Transactional memory simply treats variables in memory the same way a database treats records on disk.[1](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch6.xhtml#ch6fn1) It protects those variables with a transaction- or retry-based scheme.

A simple example of this approach is Clojure’s `atom` facility:

[Click here to view code image](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch6_images.xhtml#pch6ex04a)
...
The strategy used by `swap!` is a traditional _compare and swap_ algorithm. The value of `counter` is read and passed to `inc`. When `inc` returns, the value of `counter` is locked and compared to the value that was passed to `inc`. If the value is the same, then the value returned by `inc` is stored in `counter` and the lock is released. Otherwise, the lock is released, and the strategy is retried from the beginning.
...
This is the idea behind _event sourcing_.[2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch6.xhtml#ch6fn2) Event sourcing is a strategy wherein we store the transactions, but not the state. When state is required, we simply apply all the transactions from the beginning of time.
...
What we have learned over the last half-century is _what not to do_.
...
[1](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch6.xhtml#ch6fn-1). I know... What’s a disk?

[2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch6.xhtml#ch6fn-2). Thanks to Greg Young for teaching me about this concept.