---
id: 01JF230WSWDKT9A1KSPS8W72HF
modified: 2024-12-14T03:02:19-05:00
---
## 27  
SERVICES: GREAT AND SMALL

![Image](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780134494272/files/graphics/CH-UN27.jpg)
...
Look at that diagram of services. How many of those services will have to change to implement this feature? _All of them_. Clearly, the development and deployment of the kitty feature will have to be very carefully coordinated.

In other words, the services are all coupled, and cannot be independently developed, deployed, and maintained.

This is the problem with cross-cutting concerns. Every software system must face this problem, whether service oriented or not. Functional decompositions, of the kind depicted in the service diagram in [Figure 27.1](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch27.xhtml#ch27fig1), are very vulnerable to new features that cut across all those functional behaviors.
...
The diagram in [Figure 27.2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch27.xhtml#ch27fig2) shows the strategy. The classes in this diagram roughly correspond to the services shown in [Figure 27.1](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch27.xhtml#ch27fig1). However, note the boundaries. Note also that the dependencies follow the Dependency Rule.

Much of the logic of the original services is preserved within the base classes of the object model. However, that portion of the logic that was specific to _rides_ has been extracted into a `Rides` component. The new feature for kittens has been placed into a `Kittens` component. These two components override the abstract base classes in the original components using a pattern such as _Template Method or Strategy_.
...
![Image](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780134494272/files/graphics/27fig02.jpg)
...
Think of a service in Java as a set of abstract classes in one or more jar files. Think of each new feature or feature extension as another jar file that contains classes that extend the abstract classes in the first jar files. Deploying a new feature then becomes not a matter of redeploying the services, but rather a matter of simply _adding_ the new jar files to the load paths of those services. In other words, adding new features conforms to the Open-Closed Principle.

The service diagram in [Figure 27.3](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch27.xhtml#ch27fig3) shows the structure. The services still exist as before, but each has its own internal component design, allowing new features to be added as new derivative classes. Those derivative classes live within their own components.

![Image](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780134494272/files/graphics/27fig03.jpg)
...
What we have learned is that architectural boundaries do not fall _between_ services. Rather, those boundaries run _through_ the services, dividing them into components.

To deal with the cross-cutting concerns that all significant systems face, services must be designed with internal component architectures that follow the Dependency Rule, as shown in the diagram in [Figure 27.4](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch27.xhtml#ch27fig4). Those services do not define the architectural boundaries of the system; instead, the components within the services do.

![Image](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780134494272/files/graphics/27fig04.jpg)
...
A service might be a single component, completely surrounded by an architectural boundary. Alternatively, a service might be composed of several components separated by architectural boundaries. In rare[2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch27.xhtml#ch27fn-2) cases, clients and services may be so coupled as to have no architectural significance whatever.

[1](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch27.xhtml#ch27fn1). Therefore the number of micro-services will be roughly equal to the number of programmers.

[2](https://learning.oreilly.com/library/view/clean-architecture-a/9780134494272/ch27.xhtml#ch27fn2). We hope they are rare. Unfortunately, experience suggests otherwise.